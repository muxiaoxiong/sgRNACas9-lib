...
int readTables (char *FILEINDEX, char *FILEDATA, char *genome, char *usage, char *prname){
    ...
    if(( hashID = shmget(HASHKEY,KEY_NUM*sizeof(unsigned int),IPC_CREAT | 0644))==-1){
        fprintf(stderr, "\nAn error occured (%s) please make sure that the tables for %s aren't already loaded on memory\n",strerror(errno),genome);
        fprintf(stderr, usage, prname);
        fflush(stderr);
        return(-1);
    }
    fprintf(stderr,"%d",hashID);
...
int main (int argc, char *argv[]){
    ...
    cout << "\nTables loaded. Calculating off-target results now.\n" << endl;
